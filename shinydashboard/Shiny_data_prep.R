...
country_list <- lapply(country_list, function(x) cbind(x, "smoothed_fb_cli" = rollmean(x$fb_data.pct_covid_cli, 7, fill = NA)))
...
country_list <- lapply(country_list, function(x) cbind(x, "smoothed_fb_cli_se" = rollmean(x$fb_data.covid_se_cli, 7, fill = NA)))
